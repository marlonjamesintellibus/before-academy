...
export function RulesDiagram() {
  ...
  return (
    <figure aria-label="Interactive written-rules simulator" className="panel mt-5 p-5">
      ...
      <PredictionGate
        id="rules"
        prompt="Payment has not been approved, but the item is in stock. What will the vending machine do?"
        options={[
          { text: "Release the item", correct: false },
          { text: "Not release it", correct: true },
          { text: "Impossible to say", correct: false },
        ]}
        revealLabel="Flip the inputs and check."
      >
        ...
        <div className="mt-4 grid items-stretch gap-3 md:grid-cols-[1fr_auto_1.25fr_auto_1fr]">
          ...
          >
            ...
            <p className="mt-2 text-caption text-ink-muted">
              No prediction or judgment was needed.
            </p>
          </section>
        </div>
        <aside className="mt-4 rounded-(--radius-control) bg-surface-alt p-4 text-body">
          <strong>Notice:</strong> flip the inputs back to the same combination and the result
          repeats exactly. Complexity can add more conditions, but it does not turn a written rule
          into AI.
        </aside>
      </PredictionGate>
      <DiagramObservation>
