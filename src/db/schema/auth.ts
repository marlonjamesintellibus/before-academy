...
/** App-owned learner profile extending the auth user 1:1 (docs/engineering/database.md). */
export const learnerProfiles = pgTable(
  "learner_profiles",
  {
    ...
    userId: text("user_id")
      .notNull()
      .references(() => user.id, { onDelete: "cascade" }),
    role: learnerRole("role").notNull().default("learner"),
    ...timestamps,
  },
  (table) => [uniqueIndex("learner_profiles_user_id_idx").on(table.userId)],
);
