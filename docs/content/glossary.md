...
| Pattern-based system • | A system that decides using patterns learned from many examples |
| Deterministic output • | A result that is always the same for the same input |
| Probabilistic output • | A result based on likelihood, which can vary or be wrong |
| Machine learning | A way of building AI where systems learn patterns from examples rather than being given rules |
| Model | The learned pattern-map an AI system uses to make its outputs |
| Algorithm | A step-by-step procedure a computer follows to do a task |
| Training | Showing a system many examples so it can learn patterns |
...
| Bias | When a system's outputs unfairly favour or disadvantage certain groups or answers, often reflecting its data |
| Human review • | A person checking or deciding on a system's output before it counts |
