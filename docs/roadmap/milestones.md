...
| M2 | Lesson | content engine (10 block renderers - one per lesson-template block type, [content/lessons/README.md](../content/lessons/README.md) - rich text, record resolution), seed/publish pipeline + content-lint v1, S03 complete (depth panels, interactive diagram + text alt, glossary chips), lesson events | full lesson readable from versioned seeds; standalone-test mapping reviewed |
